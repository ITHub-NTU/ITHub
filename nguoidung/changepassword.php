...
if (!empty($_POST["submit"]) && !empty($_POST["matKhau"]) && !empty($_POST["xacNhanMK"])) {
    ...
    if (empty($matKhauLoi)) {
        $changePasswordResult = $tblNguoiDung->changePassword($newPassword, $confirmPassword);

        if ($changePasswordResult == 'Thay đổi mật khẩu thành công!') {
            header('Location: ./dangnhap.php');
            exit;
        } else {
            $confirmMessage = $changePasswordResult;
        }
    }
            ...
            <div class="card-body p-5 text-center">
                ...
                <h2 class="fw-bold mb-2 text-uppercase">Đổi mật khẩu</h2>
                    <?php if ($confirmMessage != '') { ?>
                            <p class="text-white-50 mb-5"><?php echo $confirmMessage; ?></p>                         
                    <?php } ?>
